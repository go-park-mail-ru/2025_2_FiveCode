...
import (
	...
	noteDelivery "backend/gateway_service/internal/delivery/notes"
...
func InitDeliveries(s *store.Store, conf *config.Config) *Deliveries {
	...
	layers.AuthDelivery = authDelivery.NewAuthDelivery(authGRPCClient, userGRPCClient, time.Duration(conf.Cookie.SessionDuration)*24*time.Hour)
	...
	layers.BlocksDelivery = noteDelivery.NewBlocksDelivery(blockGRPCClient)

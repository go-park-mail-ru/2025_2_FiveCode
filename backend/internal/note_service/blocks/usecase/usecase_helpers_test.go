...
func (f *fakeBlocksRepo) CreateTextBlock(ctx context.Context, noteID uint64, position float64, userID uint64) (*models.BlockWithContent, error) {
	return &models.BlockWithContent{Block: models2.Block{ID: 1, NoteID: noteID, Position: position}}, nil
}
...
func (f *fakeBlocksRepo) GetBlocksByNoteID(ctx context.Context, noteID uint64) ([]models.BlockWithContent, error) {
	return nil, nil
}
func (f *fakeBlocksRepo) GetBlockByID(ctx context.Context, blockID uint64) (*models.BlockWithContent, error) {
	return nil, nil
}
func (f *fakeBlocksRepo) UpdateBlockText(ctx context.Context, blockID uint64, text string, formats []models2.BlockTextFormat) (*models.BlockWithContent, error) {
	return nil, nil
}
func (f *fakeBlocksRepo) UpdateBlockPosition(ctx context.Context, blockID uint64, position float64) (*models2.Block, error) {
	return nil, nil
}
func (f *fakeBlocksRepo) DeleteBlock(ctx context.Context, blockID uint64) error { return nil }
func (f *fakeBlocksRepo) GetBlockNoteID(ctx context.Context, blockID uint64) (uint64, error) {
	return 1, nil
}
...
func (f *fakeNotesRepo) GetNoteById(ctx context.Context, noteID uint64, userID uint64) (*models2.Note, error) {
	return &models2.Note{ID: noteID, OwnerID: f.noteOwner}, nil
}

func Test_isDefaultFormat_and_styles_formats_merge(t *testing.T) {
	f1 := models2.BlockTextFormat{Bold: true, Font: models2.FontInter, Size: 12}
	f2 := models2.BlockTextFormat{Bold: true, Font: models2.FontInter, Size: 12}
	assert.True(t, stylesEqual(f1, f2))
	assert.False(t, isDefaultFormat(f1))

	m := map[int]models2.BlockTextFormat{0: f1, 1: {Italic: true, Font: models2.FontInter, Size: 12}}
	merged := mergeFormats(m)
	assert.True(t, merged.Bold)
	assert.True(t, merged.Italic)

	// formatsEqual considers offsets too
	fa := models2.BlockTextFormat{StartOffset: 0, EndOffset: 4, Bold: true}
	fb := models2.BlockTextFormat{StartOffset: 0, EndOffset: 4, Bold: true}
	assert.True(t, formatsEqual(fa, fb))
}

func Test_optimizeFormats_basic(t *testing.T) {
	text := "HelloWorld"
	formats := []models2.BlockTextFormat{

...
type BlocksRepository interface {
	CreateTextBlock(ctx context.Context, noteID uint64, position float64, userID uint64) (*models2.Block, error)
	CreateAttachmentBlock(ctx context.Context, noteID uint64, position float64, fileID uint64, userID uint64) (*models2.Block, error)
	CreateCodeBlock(ctx context.Context, noteID uint64, position float64, userID uint64) (*models2.Block, error)
	GetBlocksByNoteID(ctx context.Context, noteID uint64) ([]models2.Block, error)
	GetBlockByID(ctx context.Context, blockID uint64) (*models2.Block, error)
	UpdateBlockText(ctx context.Context, blockID uint64, text string, formats []models2.BlockTextFormat) (*models2.Block, error)
	UpdateBlockPosition(ctx context.Context, blockID uint64, position float64) (*models2.Block, error)
	UpdateCodeBlock(ctx context.Context, blockID uint64, language, codeText string) (*models2.Block, error)
...
type NotesRepository interface {
	GetNoteById(ctx context.Context, noteID uint64, userID uint64) (*models2.Note, error)
}
...
func (u *BlocksUsecase) CreateTextBlock(ctx context.Context, userID, noteID uint64, beforeBlockID *uint64) (*models2.Block, error) {
	log := logger.FromContext(ctx).With().
		Uint64("user_id", userID).
		Uint64("note_id", noteID).
		Str("type", models2.BlockTypeText).
		Logger()
...
func (u *BlocksUsecase) CreateAttachmentBlock(ctx context.Context, userID, noteID uint64, beforeBlockID *uint64, fileID uint64) (*models2.Block, error) {
	log := logger.FromContext(ctx).With().
		Uint64("user_id", userID).
		Uint64("note_id", noteID).
		Str("type", models2.BlockTypeAttachment).
...
func (u *BlocksUsecase) CreateCodeBlock(ctx context.Context, userID, noteID uint64, beforeBlockID *uint64) (*models2.Block, error) {
	log := logger.FromContext(ctx).With().
		Uint64("user_id", userID).
		Uint64("note_id", noteID).
		Str("type", models2.BlockTypeCode).
		Logger()

	log.Info().Msg("creating code block")
...
func (u *BlocksUsecase) UpdateBlock(ctx context.Context, userID uint64, req *models2.UpdateBlockRequest) (*models2.Block, error) {
	log := logger.FromContext(ctx).With().
		Uint64("user_id", userID).
		Uint64("block_id", req.BlockID).
		Logger()

	log.Info().Msg("updating block")
	...
	switch block.Type {
	case models2.BlockTypeCode:
		codeContent, ok := req.Content.(models2.UpdateCodeContent)
		if !ok {
	...
	case models2.BlockTypeText:
		textContent, ok := req.Content.(models2.UpdateTextContent)
		if !ok {
	...
	case models2.BlockTypeAttachment:
...
func (u *BlocksUsecase) GetBlock(ctx context.Context, userID, blockID uint64) (*models2.Block, error) {
	log := logger.FromContext(ctx).With().
		Uint64("user_id", userID).
		Uint64("block_id", blockID).
		Logger()

	log.Info().Msg("getting block")
...
func (u *BlocksUsecase) GetBlocks(ctx context.Context, userID, noteID uint64) ([]models2.Block, error) {
	log := logger.FromContext(ctx).With().
		Uint64("user_id", userID).
		Uint64("note_id", noteID).
		Logger()

	log.Info().Msg("getting blocks")
...
func (u *BlocksUsecase) UpdateBlockPosition(ctx context.Context, userID, blockID uint64, beforeBlockID *uint64) (*models2.Block, error) {
	log := logger.FromContext(ctx).With().
		Uint64("user_id", userID).
		Uint64("block_id", blockID).
		Logger()

	log.Info().Msg("updating block position")
...
func optimizeFormats(text string, formats []models2.BlockTextFormat) []models2.BlockTextFormat {
	if len(formats) == 0 {
		return []models2.BlockTextFormat{}
	}

	textLen := len(text)

	validFormats := make([]models2.BlockTextFormat, 0)
	...
	type event struct {
		offset int
		format models2.BlockTextFormat
		isEnd  bool
	}
	...
	activeFormats := make(map[int]models2.BlockTextFormat)
	result := make([]models2.BlockTextFormat, 0)
	lastOffset := 0
	formatIndex := 0
	...
	merged := make([]models2.BlockTextFormat, 0)
	current := result[0]
...
func isDefaultFormat(f models2.BlockTextFormat) bool {
	return !f.Bold && !f.Italic && !f.Underline && !f.Strikethrough &&
		f.Link == nil && f.Font == models2.FontInter && f.Size == 12
}

func formatsEqual(f1, f2 models2.BlockTextFormat) bool {
...
func stylesEqual(f1, f2 models2.BlockTextFormat) bool {
...
func mergeFormats(formats map[int]models2.BlockTextFormat) models2.BlockTextFormat {
	result := models2.BlockTextFormat{
		Font: models2.FontInter,
		Size: 12,
	}

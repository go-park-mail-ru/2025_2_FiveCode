...
func NewRouter(s *store.Store, deliveries *initialize.Deliveries) http.Handler {
	...
	ticketProtectRouter.HandleFunc("/tickets/{ticket_id}/messages", deliveries.TicketDelivery.GetTicketMessages).Methods("GET")
	ticketProtectRouter.HandleFunc("/tickets/{ticket_id}/messages", deliveries.TicketDelivery.CreateTicketMessage).Methods("POST")

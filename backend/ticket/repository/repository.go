...
func (r *TicketRepository) GetTicketByID(ctx context.Context, ticketID uint64) (*models.Ticket, error) {
	log := logger.FromContext(ctx)
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			log.Warn().Uint64("ticket_id", ticketID).Msg("ticket not found")
			return nil, fmt.Errorf("ticket not found")
		}
		log.Error().Err(err).Msg("failed to get ticket by id without user")
		return nil, err
	}
...
func (r *TicketRepository) CreateTicketMessage(ctx context.Context, message *models.TicketMessage) (*models.TicketMessage, error) {
	log := logger.FromContext(ctx)

	query := `
		INSERT INTO ticket_message (ticket_id, sender_id, sender_type, body)
		VALUES ($1, $2, $3, $4)
		RETURNING id, ticket_id, sender_id, sender_type, body, created_at
	`

	created := &models.TicketMessage{}
	err := r.db.QueryRowContext(
		ctx,
		query,
		message.TicketID,
...
func (r *TicketRepository) GetTicketMessages(ctx context.Context, ticketID uint64) ([]models.TicketMessage, error) {
	log := logger.FromContext(ctx)
	...
	if err != nil {
		log.Error().Err(err).Msg("failed to fetch ticket messages")
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			log.Error().Err(err).Msg("failed to scan ticket message")
			return nil, err
		}
	...
	if err := rows.Err(); err != nil {
		log.Error().Err(err).Msg("error iterating ticket messages")
		return nil, err
	}
